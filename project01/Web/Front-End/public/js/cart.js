...
async function loadCartPage() {
  try {
    const userRes = await fetch(`${apiBaseUrl}/auth/`, {
      method: "GET",
      credentials: "include",
    });
    ...
    const cartRes = await fetch(`${apiBaseUrl}/orders/cart/${user.id}`);
    ...
    cartItems.forEach((item) => {
      ...
      productContainer.insertBefore(
        productDiv,
        productContainer.querySelector("hr")
      );
    });
    ...
    document.querySelectorAll(".insc").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        await fetch(`${apiBaseUrl}/orders/cart/add`, {
          method: "POST",
          headers: {
    ...
    document.querySelectorAll(".desc").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const productId = btn.getAttribute("data-product-id");
        await fetch(`${apiBaseUrl}/orders/cart/decrease`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      const cartRes = await fetch(`${apiBaseUrl}/orders/cart/${user_id}`);
      if (!cartRes.ok) throw new Error("Không thể lấy giỏ hàng");
      const cartData = await cartRes.json();

      const checkoutRes = await fetch(`${apiBaseUrl}/orders/checkout`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (checkoutRes.ok) {
        await fetch(
          "https://kohvuu5txks63tcmxtcoyg3qe40tkwgi.lambda-url.us-east-1.on.aws/",
          {
            method: "POST",
